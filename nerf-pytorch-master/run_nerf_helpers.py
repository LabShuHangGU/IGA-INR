...
class param_mlplayer(nn.Module):
    def __init__(self,in_features,out_features,high_freq_num,low_freq_num,phi_num,amplitude,omega_0,siren=None):
        super(param_mlplayer,self).__init__()
    ...
    def init_bases(self):
        ...
        for freq in low_freq:
            for phi in phi_set:
                base=torch.tensor([self.amplitude*math.cos(freq*x+phi) for x in points])
                bases[i,:]=base
                i+=1
    ...
    def init_lamb(self):
        ...
        with torch.no_grad():
            ...
            for i in range(m):
                ...
                self.lamb[:,i]=nn.init.uniform_(self.lamb[:,i],-np.sqrt(6/m)/dominator,np.sqrt(6/m)/dominator)#/self.omega_0
...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        ...
        if use_viewdirs:
            ...
            # We found that only reparameterizing the last feature layer can achieve the best results in the half-res blender dataset.
            # self.feature_linear = param_mlplayer(in_features=W,out_features=W,high_freq_num=64,low_freq_num=64,phi_num=16,amplitude=0.005,omega_0=30.0)
